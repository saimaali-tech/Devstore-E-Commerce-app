...
api.get("/wishlist/:sessionId", async (req, res) => {
  ...
  try {
    ...
    res.json({
      items: items.map((w: (typeof items)[number]) => ({
        id: w.id,
        productId: w.productId,
        createdAt: w.createdAt,
        product: {
          ...w.product,
          images: parseJsonField<string[]>(w.product.images, [w.product.image]),
          tags: parseJsonField<string[]>(w.product.tags, []),
        },
      })),
    });
  } catch (e) {
